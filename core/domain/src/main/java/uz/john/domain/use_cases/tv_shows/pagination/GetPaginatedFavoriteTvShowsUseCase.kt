...
import uz.john.data.pagination.tv_shows.FavoriteTvShowsPagingSource
...
class GetPaginatedFavoriteTvShowsUseCase @Inject constructor(
    private val tvShowsRepository: TvShowsRepository
) {
    operator fun invoke(): Flow<PagingData<TvShow>> = Pager(
        config = PagingConfig(pageSize = 20),
        pagingSourceFactory = {
            FavoriteTvShowsPagingSource(
